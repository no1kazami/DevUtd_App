...
#include "BWidget_PlayerChart.generated.h"

class UBWidget_EquipFilterItem;

UCLASS()
class BIGTAMIN_API UBWidget_PlayerChart : public UBWidget
{
	GENERATED_BODY()
private:
	...
	UPROPERTY()	FString _PlayerName;
	UPROPERTY()	TArray<FST_PLAYER_CHART_INFO_DATA> _DayMatchData;
	UPROPERTY()	TArray<FST_PLAYER_CHART_INFO_DATA> _WeekMatchData;
	UPROPERTY()	TArray<FST_PLAYER_CHART_INFO_DATA> _MonthMatchData;
	...
	UPROPERTY()	E_SORT_TYPE _ChartType = E_SORT_TYPE::E_SORT_TYPE_GOAL;
...
public:
	...
	UFUNCTION()	void SetPlayerData( E_SORT_TYPE type, const FString PlayerName );
